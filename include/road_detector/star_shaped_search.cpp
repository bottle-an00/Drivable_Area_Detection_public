...
float params::angleFilter3 = 40;     //"slope" parameter for edge detection (in degrees, given by 2 points, in radial direction/plane)
float slope_param;              //same but in radians
float params::kdev_param=1.425;            //coefficient: weighting the impact of deviation (difference) from average ( = overall sensitivity to changes)
float params::kdist_param=2;              //coefficient: weighting the impact of the distance between points ( = sensitivity for height error at close points)
bool params::starbeam_filter = false;           //toggle usage of rectangular beams for starshaped algorithm instead of the whole sector (containing the beam)
int params::dmin_param = 10;                 //minimal number of points required to begin adaptive evaluation


std::vector<box> beams(rep);        //beams
std::vector<box *> beamp(rep + 1);  //pointers to the beams (+1 -> 0 AND 360)

bool ptcmpr(polar_ a, polar_ b)   //comparison by r-coordinates
{
    return (a.r < b.r);
}
...
void Detector::starShapedSearch(std::vector<Point2D> &array2D)  //entry point to the code, everything gets called here (except for initialization - that needs to be called separately, at the start of the program - "beam_init()")
{
    ...
    float r, fi;                    //polar_ coordinates
    slope_param = params::angleFilter3 * (M_PI / 180);

    for (int i = 0; i < s; i++) //points to polar_ coordinate-system + sorting into sectors
    {
        ...
        beamp[f]->p.push_back(polar_{i, r, fi}); //adding the point to the 'f'-th beam (still unfiltered)
    }
