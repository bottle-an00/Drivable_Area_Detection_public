...
std::string params::fixedFrame = "base_link";             //Fixed Frame.
bool        params::x_zero_method = true;
bool        params::z_zero_method = true;
bool        params::star_shaped_method = true;     //methods of roadside detection
float       params::interval = 0.5800;               //acceptable interval for the LIDAR's vertical angular resolution
...
void Detector::filtered(pcl::PointCloud<pcl::PointXYZI>::Ptr ground_cloud, 
pcl::PointCloud<pcl::PointXYZI>::Ptr Road_cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr Curb_cloud){
    ...
    size_t piece = ground_cloud->points.size();
    ...
    for (i = 0; i < piece; i++){
        ...
        array2D[i].p.x = ground_cloud->points[i].x;
        array2D[i].p.y = ground_cloud->points[i].y;
        array2D[i].p.z = ground_cloud->points[i].z;
        array2D[i].p.intensity = ground_cloud->points[i].intensity;
    ...
    for (i = 0; i < piece; i++)
    {
        ...
        if (results == 1)
        {
            ...
            array3D[j][indexArray[j]].p.intensity = array2D[i].p.intensity;
    ...
    *Road_cloud = cloud_filtered_Road;
    *Curb_cloud = cloud_filtered_High;
