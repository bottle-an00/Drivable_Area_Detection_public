...
#include "patchworkpp/drivable_ground.h"
#include "patchworkpp/object_detection.h"

using PointTypeR = PointXYZIR;
using namespace std;

boost::shared_ptr<PatchWorkpp<PointTypeR>> PatchworkppGroundSeg;
...
void callbackCloud(const sensor_msgs::PointCloud2ConstPtr &cloud_msg)
{
    ...
    pcl::PointCloud<PointTypeR> pc_curr;
    pcl::PointCloud<PointTypeR> pc_ground;
    pcl::PointCloud<PointTypeR> pc_non_ground;

    pcl::fromROSMsg(*cloud_msg, pc_curr);

    PatchworkppGroundSeg->estimate_ground(pc_curr, pc_ground, pc_non_ground, time_taken);
    ROS_INFO_STREAM("\033[1;32m" << "cloud fixed frame :: "  << pc_curr.header.frame_id<< "\033[0m");
...
int main(int argc, char**argv) {
    ...
    PatchworkppGroundSeg.reset(new PatchWorkpp<PointTypeR>(&pnh));
